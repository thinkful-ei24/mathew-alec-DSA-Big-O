...
//Why: It seems to just be counting down from n.
...
//Complexity: O(n) or Linear
//Why: Only runs the length of arr times.
...
//Complexity: O(n) or Linear
//Why: Only runs the length of the string.
...
//Why: It will call convertToBinary on nth time, n/2 time, n/4 time, etc.
...
//Complexity: O(n) or Linear
//Why: Only runs the length of the n.
...
//Complexity: O(2^n) or ??
//Why: It has to grow quickly because each time fibonacci runs, it calls two more fibonacci calls.
...
//Complexity: O(n^n) or  ???
//Why: For each for I loop, it runs anagrams again. 
...
//Complexity: O(n^2) or Quadratic maybe O(2^n)???
//Why: It's traversing a tree so I'm not sure what that is...
...
//Complexity: O(2^n) or Exponential time
//Why: Only runs the length of the string.
